...
function EnforceCommanderSafety(aiBrain, now)
    ...
    local lastHealthRatio = runtime.LastAcuHealthRatio or healthRatio
    if healthRatio < (lastHealthRatio - 0.015) then
        runtime.LastAcuDamageTime = now
    end
    runtime.LastAcuHealthRatio = healthRatio
    ...
    local techAnchorPosture = (macroObjective == 'first_land_hq' or macroObjective == 'first_t2_engineer' or macroObjective == 'first_t2_power')
        and distance <= math.max(34, maxDistance + 18)
        and escortCount >= 6
        and healthRatio >= 0.85
        and localThreat <= (homeThreat + 4.2)
        and enemyRaiders <= 2
        and not lowHealth
        and not enemyContactUnsafe
        and not raidRecall
    ...
    if shouldRecall or idleFar or stuckFar or noMansLand or enemyContactUnsafe or raidRecall or leashReposition then
        ...
        if recallAction == 'combat_retreat'
            and retreatingSuccessfully
            and sinceRecall < 12 then
            runtime.LastAcuDistanceFromBase = distance
            return
        end
